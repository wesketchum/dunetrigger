...
public:
  ...
  TriggerCandidateMakerOnlineTPC(TriggerCandidateMakerOnlineTPC const&) = delete;
  TriggerCandidateMakerOnlineTPC(TriggerCandidateMakerOnlineTPC&&) = delete;
  TriggerCandidateMakerOnlineTPC& operator=(TriggerCandidateMakerOnlineTPC const&) = delete;
  TriggerCandidateMakerOnlineTPC& operator=(TriggerCandidateMakerOnlineTPC&&) = delete;
...
private:
  ...
  std::shared_ptr<triggeralgs::AbstractFactory<triggeralgs::TriggerCandidateMaker>> alg_factory = triggeralgs::TriggerCandidateFactory::get_instance();
  std::unique_ptr<triggeralgs::TriggerCandidateMaker> alg;

  typedef std::pair<size_t, dunedaq::trgdataformats::TriggerActivityData> TriggerActivityIdx;

  static bool compareTriggerActivity(const dunedaq::trgdataformats::TriggerActivityData& ta1, const dunedaq::trgdataformats::TriggerActivityData& ta2)
    { return (ta1.time_start < ta2.time_start); }

  static bool isTAEqual(const dunedaq::trgdataformats::TriggerActivityData& ta1, const dunedaq::trgdataformats::TriggerActivityData& ta2)
    { return (ta1.detid == ta2.detid && ta1.channel_peak == ta2.channel_peak && ta1.time_peak == ta2.time_peak && ta1.adc_integral == ta2.adc_integral); }

};

duneana::TriggerCandidateMakerOnlineTPC::TriggerCandidateMakerOnlineTPC(fhicl::ParameterSet const& p)
  : EDProducer{p},
  ta_tag(p.get<art::InputTag>("ta_tag")),
...
void duneana::TriggerCandidateMakerOnlineTPC::produce(art::Event& e)
{
  ...
  auto ta_in_tc_assn_ptr = std::make_unique<art::Assns<TriggerCandidateData, TriggerActivityData>>();
  ...
  std::sort(input_tas.begin(), input_tas.end(),
    [](TriggerActivityIdx& ta1, TriggerActivityIdx& ta2){return compareTriggerActivity(ta1.second, ta2.second);});
  ...
  for(auto out_tc : produced_tcs){
    ...
    for(const TriggerActivityData& in_ta : out_tc.inputs){
      ...
      std::vector<TriggerActivityIdx>::iterator ta_it =
        std::find_if(input_tas.begin(), input_tas.end(), [&](TriggerActivityIdx& t){return isTAEqual(in_ta, t.second);});
      // stop when there are no more matches
      while (ta_it != input_tas.end()){
        // push back the matching ptr
        tas_in_tc_ptr.push_back(art::Ptr<TriggerActivityData>(ta_handle, ta_it->first));
        // and get an iterator pointing to the next match
        ta_it =
          std::find_if(++ta_it, input_tas.end(), [&](TriggerActivityIdx& t){return isTAEqual(in_ta, t.second);});
      }
    }
